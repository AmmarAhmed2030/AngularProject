...
calculateAge(birthDateString:string) {
  ...
  getPrescriptionData(AppointmentId:any){
    this.prescriptionService.getAppointmentById(AppointmentId).subscribe(appointment=>{
  ...
  getPatientData(patientId:any){
    this.prescriptionService.getPatientById(patientId).subscribe(patient=>{
      console.log(patient);
      this.prescription.patientAge=Number(this.calculateAge(patient[0].dateOfBirth));
